...
```{r logtable1, tab.caption = "Regression with *Booking Type_Private*"}
logtable1 <- tidy(logmodel)%>%
  mutate(p.value = stars.pval(p.value))%>%
  mutate("'Exam Result_PASS'" = paste(estimate, p.value, sep = " "))%>%
  select("'Exam Result_PASS'")%>%
  as.data.frame()

rownames(logtable1) <- c("Constant","Booking Type_Private")

tabl1stat <- glance(logmodel)%>%
  select(c(nobs, AIC, logLik))%>%
  t() %>%
  as.data.frame()
colnames(tabl1stat) <- "'Exam Result_PASS'"
...
kable(logtable1,
  ...
  pack_rows(" ", 3, 5)%>%
  footnote(general = "+ p≤0.1; * p≤0.5; ** p≤0.01; *** p≤0.001") %>%
  kable_styling(bootstrap_options = "striped")

```

Table \@ref(tab:logtable1) shows the regression summary. *Booking Type_Private* has p-value close to 0 which means it is statistically significant. Due to the variable being a dummy variable relative to booking type driving school, the coefficient indicates that *Booking Type_Private* affects the passing of an exam negatively compared to *Booking Type_Driving School*. Private booking reduces the log odds by 0.061.
...
```{r comparison, tab.caption= "Regression Results",results = "asis", eval =FALSE}
